...
export function CaseDetail({ caseId, onBack }: CaseDetailProps) {
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center py-16 px-6 bg-red-500/10 border border-red-500/20 rounded-xl max-w-xl mx-auto text-center space-y-3">
        ...
        <h3 className="text-lg font-bold text-foreground font-['Space_Grotesk']">Case Locked</h3>
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-background text-foreground overflow-hidden">
      ...
      {/* Header */}
      <div className="p-8 border-b border-border flex items-center gap-4 shrink-0 bg-card text-card-foreground/50 backdrop-blur-md">
        <button onClick={onBack} className="p-2 hover:bg-accent rounded-lg transition-colors cursor-pointer text-foreground">
          <ArrowLeft size={20} />
        </button>
        <div>
          <div className="flex items-center gap-3">
            <h1 className="text-2xl font-bold tracking-tight text-foreground font-['Space_Grotesk']">{caseData.case_number}</h1>
      ...
      <div className="flex-1 overflow-auto flex flex-col lg:flex-row gap-6 p-8">
        ...
        <div className="flex-1 flex flex-col gap-6">
          <div className="bg-card text-card-foreground/70 border border-border p-6 rounded-2xl space-y-4">
            <h2 className="text-base font-bold text-foreground uppercase tracking-wider border-b border-border pb-2">Investigation Narrative</h2>
            <p className="text-foreground whitespace-pre-wrap text-sm leading-relaxed font-sans">
          ...
          <div className="bg-card text-card-foreground/70 border border-border p-6 rounded-2xl flex-1 overflow-auto">
            <h3 className="text-base font-bold text-foreground uppercase tracking-wider border-b border-border pb-2 mb-4 flex items-center gap-2">
            ...
            <div className="space-y-6 pl-4 border-l border-border">
              ...
              {timeline.length === 0 && (
                <div className="text-center text-muted-foreground py-8 text-sm">No activity recorded.</div>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="w-full lg:w-[350px] flex flex-col gap-6 shrink-0">
          ...
          {canInvestigate && (
            <div className="bg-card text-card-foreground/70 border border-border p-6 rounded-2xl space-y-4">
              <h3 className="text-sm font-bold text-foreground uppercase tracking-wider border-b border-border pb-2">Investigation Controls</h3>
              ...
              <form onSubmit={handleUpdate} className="space-y-4">
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-background border border-border rounded-lg py-2 px-3 text-foreground text-xs focus:outline-none focus:border-[#00e07a]/50 disabled:opacity-50"
                  >
          ...
          <div className="bg-card text-card-foreground/70 border border-border p-6 rounded-2xl text-xs space-y-4">
            <h3 className="text-sm font-bold text-foreground uppercase tracking-wider border-b border-border pb-2">Case Metadata</h3>
            
            <div className="space-y-4 font-sans text-foreground">
              <div>
                <span className="text-muted-foreground block mb-1 uppercase font-bold text-[9px] tracking-wider">Subject Employee ID</span>
              ...
              <div>
                <span className="text-muted-foreground block mb-1 uppercase font-bold text-[9px] tracking-wider">Severity Tier</span>
                <span className="text-orange-400 font-medium">{caseData.severity}</span>
              </div>
              <div>
                <span className="text-muted-foreground block mb-1 uppercase font-bold text-[9px] tracking-wider">Reported By</span>
                <span>{caseData.anonymous_report === 1 ? "Anonymous" : (caseData.reported_by_employee_id || "Direct File")}</span>
              </div>
              <div>
                <span className="text-muted-foreground block mb-1 uppercase font-bold text-[9px] tracking-wider">Created At</span>
                <span className="font-mono text-muted-foreground">{new Date(caseData.created_at).toLocaleString()}</span>
              </div>
              {caseData.date_closed && (
                <div>
                  <span className="text-muted-foreground block mb-1 uppercase font-bold text-[9px] tracking-wider">Date Closed</span>
