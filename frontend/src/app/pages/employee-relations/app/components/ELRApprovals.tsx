        ...
        <div className="space-y-4">
          {approvals.length === 0 ? (
            ...
            approvals.map(approval => (
              <div key={approval.id} className="bg-card border border-border rounded-xl p-5 shadow-sm">
                <div className="flex justify-between items-start">
                  <div>
                    <div className="flex items-center gap-3 mb-2">
                      <h3 className="text-lg font-bold text-foreground">{approval.subject}</h3>
                  ...
                  {approval.status === 'Pending' && (
                    <div className="flex flex-col items-end gap-2">
                      {decidingId === approval.id ? (
                        <div className="bg-muted border border-border p-3 rounded-xl w-72">
                          <textarea 
                            value={decisionNote}
                            onChange={(e) => setDecisionNote(e.target.value)}
                            placeholder="Add a note (optional)"
                            className="w-full bg-card border border-gray-200 dark:border-white/5 rounded p-2 text-xs mb-2 focus:outline-none focus:border-[#00e07a]"
                            rows={2}
                          ></textarea>
                        ...
                        <div className="flex gap-2">
                          <button onClick={() => setDecidingId(approval.id)} className="px-4 py-2 border border-border hover:bg-accent rounded-lg text-sm font-bold transition-colors">Make Decision</button>
                        </div>
                      )}
                    </div>
                  )}
                  ...
                  {approval.status !== 'Pending' && approval.decision_note && (
                    <div className="max-w-xs text-xs text-muted-foreground bg-muted p-3 rounded-lg border border-border">
