...
interface TicketListProps {
  ...
  onFilterCompany: (company: string | "all") => void;
}
...
export function TicketList({ tickets, selectedId, onSelect, onNew, filterStatus, onFilterStatus, filterCompany, onFilterCompany }: TicketListProps) {
  const [query, setQuery] = useState("");

  const companies = Array.from(new Set(tickets.map(t => t.tags[0] || "General"))).sort();

  const filtered = tickets.filter(t => {
    const matchStatus = filterStatus === "all" || t.status === filterStatus;
    const matchCompany = filterCompany === "all" || t.tags.includes(filterCompany);
      ...
      <div style={{ padding: "1.25rem 1rem 0.875rem", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "0.875rem" }}>
          ...
          <button
            ...
            style={{
              ...
              fontWeight: 700,
              cursor: "pointer",
              boxShadow: "0 2px 12px rgba(0, 224, 122, 0.2)",
              fontFamily: "'JetBrains Mono', monospace",
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
          <div style={{ display: "flex", gap: "0.25rem", flexWrap: "wrap" }}>
            {STATUS_FILTERS.map(f => (
              <button
                ...
                style={{
                  background: filterStatus === f.key ? "rgba(0, 224, 122, 0.15)" : "transparent",
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
            <SlidersHorizontal size={12} color="#4b5a6e" />
            <select
              value={filterCompany}
              ...
              style={{
                background: "rgba(255,255,255,0.05)",
                border: "1px solid rgba(255,255,255,0.08)",
                color: filterCompany === "all" ? "#8899b4" : "#00e07a",
                borderRadius: "0.375rem",
                padding: "0.25rem 0.5rem",
                fontSize: "0.6875rem",
                fontFamily: "'JetBrains Mono', monospace",
            ...
            >
              <option value="all" style={{ background: "#0b0f1a", color: "#8899b4" }}>All Companies</option>
              {companies.map(c => (
                <option key={c} value={c} style={{ background: "#0b0f1a", color: "#f0f4ff" }}>{c}</option>
              ))}
            </select>
          </div>
        </div>
            ...
            return (
              <div
                ...
                style={{
                  ...
                  background: active ? "rgba(0, 224, 122, 0.05)" : "transparent",
                  cursor: "pointer",
                  transition: "all 0.15s ease",
                }}
