...
export function Scheduling() {
  ...
  return (
    <div className="flex flex-col h-full bg-background text-muted-foreground p-6 font-['Inter',sans-serif]">
      {/* Header */}
      ...
      {/* Controls Bar */}
      <div className="flex justify-between items-center bg-card text-card-foreground p-4 rounded-lg border border-border mb-6">
        <div className="flex items-center gap-4">
          ...
          <div className="font-semibold text-lg text-foreground font-['Space_Grotesk',sans-serif] min-w-[250px] text-center">
      ...
      {/* Roster Table */}
      <div className="bg-card text-card-foreground rounded-lg border border-border overflow-x-auto flex-1">
        <table className="w-full min-w-[1000px] border-collapse">
          <thead>
            <tr>
              <th className="sticky left-0 bg-card text-card-foreground z-10 p-4 text-left text-xs font-semibold uppercase text-muted-foreground border-b border-r border-border w-[250px]">
                Employee
              </th>
              {dates.map((d, i) => {
                const isToday = formatDateYMD(d) === formatDateYMD(new Date());
                return (
                  <th key={i} className={`p-4 text-left text-xs font-semibold uppercase border-b border-r border-border ${isToday ? 'text-[#3b82f6]' : 'text-muted-foreground'}`}>
                    {formatDisplayDate(d)}
                  </th>
                );
              })}
          ...
          <tbody>
            {isLoading ? (
              <tr><td colSpan={8} className="text-center p-10 text-muted-foreground">Loading Roster...</td></tr>
            ) : employees.length === 0 ? (
              <tr><td colSpan={8} className="text-center p-10 text-muted-foreground">No employees found.</td></tr>
            ) : (
              employees.map(emp => {
                const initials = emp.full_name.split(' ').map(n => n[0]).join('').substring(0, 2).toUpperCase();
                return (
                  <tr key={emp.user_id}>
                    <td className="sticky left-0 bg-card text-card-foreground z-[1] p-3 border-b border-r border-border">
                      <div className="flex items-center gap-3">
                        <div className="w-9 h-9 rounded-full bg-background flex items-center justify-center font-bold text-foreground overflow-hidden shrink-0">
                          {emp.profile_image ? (
                    ...
                    {dates.map((d, i) => {
                      ...
                      const activeClass = currentVal > 0 ? 'bg-blue-500/10 border-blue-500 text-blue-400 font-semibold' : 'bg-card text-card-foreground border-border text-foreground';

                      return (
                        <td key={i} className="p-3 border-b border-r border-border bg-background align-top">
                          <select 
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center">
          <div className="bg-card text-card-foreground w-[450px] rounded-xl border border-border shadow-2xl overflow-hidden flex flex-col">
            ...
            <div className="p-5">
              ...
              <div className="mb-3">
                ...
                <input 
                  ...
                  className="w-full bg-input border-border border border-border rounded-md px-3 py-2 text-sm text-foreground focus:outline-none focus:border-[#00e07a]"
                />
              </div>
            ...
            <div className="p-5 border-t border-border flex justify-end gap-3">
              <button 
                className="px-4 py-2 border border-border hover:bg-accent rounded-md text-foreground text-sm transition-colors"
