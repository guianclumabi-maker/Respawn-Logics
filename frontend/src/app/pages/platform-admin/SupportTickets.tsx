    ...
    <div className="p-8 max-w-7xl mx-auto">
      ...
      <div className="flex gap-3 mb-5">
        ...
        <select
          ...
          className="px-3 py-2 bg-[#0c1018] border border-white/[0.07] rounded-lg text-sm text-slate-400 focus:outline-none focus:border-violet-500/40"
        >
          ...
          <tbody>
            ...
            {!loading && tickets.map((t) => (
              <tr key={t.id} className="border-b border-white/[0.03] hover:bg-white/[0.02] transition-colors">
                ...
                <td className="px-5 py-4 text-slate-400 text-sm">{t.company_name}</td>
                <td className="px-5 py-4">
                  <span className={`px-2 py-0.5 rounded-full text-xs font-medium border capitalize ${PRIORITY_COLORS[t.priority?.toLowerCase()] ?? "bg-slate-700 text-slate-400 border-slate-600"}`}>
                    {t.priority}
                  </span>
                </td>
                <td className="px-5 py-4">
                  <select
                    ...
                    className={`px-2 py-1 rounded-lg text-xs font-medium border-none outline-none cursor-pointer capitalize ${STATUS_COLORS[t.status?.toLowerCase().replace(" ", "_")] ?? "bg-slate-700 text-slate-400"}`}
                  >
                ...
                <td className="px-5 py-4">
                  <span className={`text-xs font-medium flex items-center gap-1 ${SLI_COLORS[t.sli_status] ?? "text-slate-400"}`}>
                    <AlertCircle size={12} />
                    {t.sli_status}
                  </span>
                </td>
